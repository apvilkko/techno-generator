...
import { createCompressor, createVCA } from "./components";
...
export const addInsert = (ctx, key, insertEffect, index = -1) => {
  ...
  const sourceGain = isMaster ? mixBus(tracks) : tracks[key].gain;
  const dest = isMaster ? tracks[key].fxloop : mixBus(tracks);
...
export const createMixer = (ctx, trackSpec) => {
  ...
  const masterFxLoop = createVCA({
    context,
    gain: 1,
    destination: masterLimiter
  });
  const mixBus = createVCA({
    context,
    gain: 0.4,
    destination: masterFxLoop
  });
  const tracks = {
    master: createTrack({
      gain: masterGain,
